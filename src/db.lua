...
---@field private node_id number 节点id
---@field byte_order string 字节序
---@field block_size number 簇大小
---@field addr_size number 地址长度
---@field can_each boolean 遍历支持
---@field buffer_size number 缓冲区大小
---@field ver number 数据库版本
---@field BIT_16 number 地址16位
---@field BIT_24 number 地址24位
---@field BIT_32 number 地址32位
---@field BIT_48 number 地址48位
---@field BIT_64 number 地址64位
...
---@class LUADB_ADDR
---@field pointer number 指针地址
---@field addr number 成员地址
---@field key_size number key长度
...
local type, pairs, tostring, setmetatable, getmetatable, error, assert, load, next, tonumber = type, pairs, tostring,
    setmetatable, getmetatable, error, assert, load, next, tonumber
local math_type, string_dump, table_concat, string_byte, table_unpack, table_insert = math.type, string.dump,
    table.concat, string.byte, table.unpack, table.insert
...
function db:init()
    ...
    if self.buffer_size <= 0 then
        ...
        self.fw:setvbuf('full', self.buffer_size)
        self.fg:setvbuf('full', self.buffer_size)
        self.fm:setvbuf('full', self.buffer_size)
    end
    -- 校验文件标识，并赋值版本号
...
function db:id(key)
    ...
    if getmetatable(key) == db.TYPE_ADDR then
        local o = { pointer = key.pointer, key = key.key, name = key.name, level = key.level }
        return setmetatable(o, db.TYPE_ID)
    end
    -- 获取指针地址
    local po, _, _, _, _, level = self:get_pointer(key)
...
--- 加载成员指针
---@param key any 成员key
---@param po number 指针地址
---@param level number 指针深度
---@return LUADB_ID
function db:load_id(key, po, level)
...
function db:addr(id)
    ...
    if getmetatable(id) ~= db.TYPE_ID then
        -- 获取指针地址
        local po, addr, size, _, _, level = self:get_pointer(id)
        local name = id -- 处理成员名称
        if self.node_id > 6 then
            name = name:sub(self.addr_size + 1)
        end
        -- 创建成员地址对象
        local o = { pointer = po, addr = addr, key_size = size, key = id, name = name, level = level }
        return setmetatable(o, db.TYPE_ADDR)
    end
    -- 否则获取成员地址
    local addr, size = self:get_addr(id.pointer, id.key)
    -- 创建成员地址对象
    local o = { pointer = id.pointer, addr = addr, key_size = size, key = id.key, name = id.name, level = id.level }
    return setmetatable(o, db.TYPE_ADDR)
end
...
---@private
---@param key any 成员key
---@return number,number,number,any,nil,number
...
--- 获取地址
---@private
---@param pointer number 指针地址
---@param key any 成员key
---@return number,number
---@overload fun(pointer:any):number,number
...
---@return number,number,number,string,LUADB_ADDR,number
...
---@return LuaDB
function db:buff(size)
    self.buffer_size = size or 0
    if self.buffer_size <= 0 then
        self.fw:setvbuf('no')
        self.fg:setvbuf('no')
        self.fm:setvbuf('no')
...
---@private
function db.TYPE_ID:__tostring()
    return string.format('LuaDB @id: 0x%x', self.pointer)
end

---@private
function db.TYPE_ADDR:__tostring()
    return string.format('LuaDB @addr: 0x%x', self.addr)
end
